...
dir_of_run=/ptmp/iclight/eager/mapping/pestis_evolution_production_runs/pestis_evolution_modern_production_run_Ypestis_pseudotb_enterocolitica_pangenome
...
for SLURM_ARRAY_TASK_ID in $(seq 1 135)
    ...
    bam=temp_for_merging/${SLURM_ARRAY_TASK_ID}_modern/${sample_name}_rmdup_mapped_merged.bam
    ...
    bam=temp_for_merging/${SLURM_ARRAY_TASK_ID}_modern/${sample_name}_rmdup_mapped_qual20_merged.bam
    samtools view -@ 72 -F4 -q 20 -b temp_for_merging/${SLURM_ARRAY_TASK_ID}_modern/${sample_name}_rmdup_mapped_merged.bam > ${bam}
